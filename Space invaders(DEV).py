    ...
    def Controls(self):
        for event in pygame.event.get():
            if event.type == KEYDOWN:
                ...
                #When P is pressed a power up is dropped.(For Testing purposes)        
                if event.key == K_p:
                    PowerUps.Drop(100,100)

                #When T is pressed the hitboxes are shown for all objects.(For Testing purposes)    
                if event.key == K_t:
                    if Alien.MoveBox.Toggle == True:
                        Alien.MoveBox.Toggle = False
                    elif Alien.MoveBox.Toggle == False:
                        Alien.MoveBox.Toggle = True

                #When X is pressed a life is lost.(For Testing purposes)
                if event.key == K_x:
                    HUD.Lives -=1

                #When Z is pressed a Level increases.(For Testing purposes)
                if event.key == K_c:
                    HUD.Level +=1
        ...
        def Shooting():
            ...
            if len(Player.Bullet.List) > 0:
                ...
                for item in Player.Bullet.List:
                    ...
                    #Draws the hitboxes to the screen if they are toggled.
                    if Alien.MoveBox.Toggle == True:
                        pygame.draw.rect(Screen.Window,RED,Player.Bullet.List[Player.Bullet.List.index(item)], 2)
    ...
    class Bullet:
        ...
        def Shoot():
            ...
            if Alien.Bullet.Loaded == False:
                ...
                if Alien.MoveBox.Toggle == True:
                    pygame.draw.rect(Screen.Window,RED,Hitbox, 2)
            else:
